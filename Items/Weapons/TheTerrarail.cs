...
{
	public class TheTerrarail : ModItem
	{
		public override void SetStaticDefaults()
		{
			Tooltip.SetDefault("A very powerful railgun. Right-click to toggle between burst-fire and full auto.");
		}

		public override void SetDefaults()
		{
			item.damage = 550; // Sets the item's damage. Note that projectiles shot by this weapon will use its and the used ammunition's damage added together.
			...
			item.value = 35000; // how much the item sells for (measured in copper)
			item.rare = ItemRarityID.Yellow; // the color that the item's name will be in-game
			item.UseSound = SoundID.Item92; // The sound that this item plays when used.
			item.autoReuse = true; // if you can hold click to automatically use it again
			item.shoot = //idk why but all the guns in the vanilla source have this
			(int)(item.shootSpeed = 35f); // the speed of the projectile (measured in pixels per frame)
			item.useAmmo = ModContent.ItemType<EnergyShell>(); // The "ammo Id" of the ammo item that this weapon uses. Note that this is not an item Id, but just a magic value.
		}
		...
		public override void AddRecipes()
		{
			...
			recipe.AddIngredient(ModContent.ItemType<LumiphyteRailgun>());
			recipe.AddIngredient(ModContent.ItemType<AmberRailgun>());
			recipe.AddIngredient(ItemID.Emerald, 35);
			recipe.AddIngredient(ItemID.Cog, 5);
			recipe.AddIngredient(ItemID.Wire, 20);
			recipe.AddIngredient(ItemID.GreenWrench, 20);
			recipe.AddTile(TileID.MythrilAnvil);
			recipe.SetResult(ModContent.ItemType<TheTerrarail>());
