...
{
	public class LuminiteRailgun : ModItem
	{
		public override void SetStaticDefaults()
		{
			Tooltip.SetDefault("An upgraded railgun. Most stats have been improved.");
		}

		public override void SetDefaults()
		{
			item.damage = 375; // Sets the item's damage. Note that projectiles shot by this weapon will use its and the used ammunition's damage added together.
			...
			item.value = 25000; // how much the item sells for (measured in copper)
			item.rare = ItemRarityID.Cyan; // the color that the item's name will be in-game
			item.UseSound = SoundID.Item68; // The sound that this item plays when used.
			item.autoReuse = false; // if you can hold click to automatically use it again
			item.shoot = //idk why but all the guns in the vanilla source have this
			(int)(item.shootSpeed = 25f); // the speed of the projectile (measured in pixels per frame)
		...
		public override void AddRecipes()
		{
			...
			recipe.SetResult(ModContent.ItemType<LuminiteRailgun>());
